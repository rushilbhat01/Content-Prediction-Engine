...
def model_exists(niche: str) -> bool:
    """Check if a trained model exists for this niche."""
    return (MODELS_DIR / f"xgb_{niche}.pkl").exists()
...
with st.form("predict_form"):
    ...
    # Niche selector — show ready niches enabled, others disabled
    ready_niches   = [k for k, v in NICHES.items() if v["ready"] and model_exists(k)]
    all_niches     = list(NICHES.keys())

    niche_options  = [k for k in all_niches if NICHES[k]["ready"] and model_exists(k)]
    niche_labels   = [f"{NICHES[k]['emoji']}  {NICHES[k]['label']}" for k in niche_options]

    if not niche_options:
        st.warning("No trained models found yet. Run model_train.py first.")
        st.stop()
    ...
    niche_idx = st.selectbox(
        ...
        index=0,
        help="Select the niche that best matches your content. Only niches with trained models are shown.",
    )
    selected_niche = niche_options[niche_idx]
    niche_cfg      = NICHES[selected_niche]

    # Coming-soon niches
    coming_soon = [k for k in all_niches if not (NICHES[k]["ready"] and model_exists(k))]
    if coming_soon:
        labels = ", ".join(f"{NICHES[k]['emoji']} {NICHES[k]['label']}" for k in coming_soon)
        st.caption(f"Coming soon: {labels}")
    ...
    channel    = st.text_input(
        ...
        placeholder="@YourChannel — used to benchmark against your own avg views"
    )
...
if submitted:
    ...
    spinner_msg = f"Analysing your {niche_cfg['emoji']} {niche_cfg['label']} Short... ~30-60 seconds"
    ...
    if display_pct is not None:
        ...
        niche_color = niche_cfg["color"]
        niche_label = niche_cfg["label"]
        niche_emoji = niche_cfg["emoji"]
...
<div class="score-wrap">
  ...
  <div class="score-sub">
    Percentile vs {verdict.lower()} videos in
    <span class="niche-badge" style="background:{niche_color}22; color:{niche_color}">
      {niche_emoji} {niche_label}
    </span>
  </div>
</div>
""", unsafe_allow_html=True)
